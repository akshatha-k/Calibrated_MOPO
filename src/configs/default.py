...
from src.modeling.models import BNN  # removed NN and TFGP
...
def create_config(env_name, ctrl_type, ctrl_args, overrides, logdir):
    ...
    type_map = DotMap(
        exp_cfg=DotMap(
            sim_cfg=DotMap(task_hor=int, stochastic=make_bool, noise_std=float),
            exp_cfg=DotMap(
    ...
    if ctrl_type == "MPC":
        ...
        if ctrl_args.get("model-type", "PE") in ["P", "PE", "D", "DE"]:
            ...
            if len(ctrl_args["model-type"]) == 1:
                ...
                type_map.ctrl_cfg.prop_cfg.model_init_cfg.num_nets = create_read_only(
                    "Number of nets for non-ensembled nets must be one, do not modify."
                )
            else:
                model_init_cfg.num_nets = 5
                type_map.ctrl_cfg.prop_cfg.model_init_cfg.num_nets = create_conditional(
                    int,
                    lambda string: int(string) > 1,
        ...
        if ctrl_cfg.prop_cfg.mode[:2] == "TS":
            ...
            if ctrl_args["model-type"] == "DE":
                ...
                type_map.ctrl_cfg.prop_cfg.ign_var = create_read_only(
                    "DE-TS* methods must ignore variance, do not modify."
                )
        if ctrl_cfg.prop_cfg.mode == "E":
            ctrl_cfg.prop_cfg.npart = 1
            type_map.ctrl_cfg.prop_cfg.npart = create_read_only(
                "Only need one particle for deterministic propagation, do not modify."
            )
        if ctrl_args["model-type"] == "D" and ctrl_cfg.prop_cfg.mode != "E":
            raise ValueError(
                "Can only use deterministic propagation for deterministic models."
            )
        ...
        if ctrl_cfg.opt_cfg.mode == "CEM":
            ...
            type_map.ctrl_cfg.opt_cfg.cfg = DotMap(popsize=int)
        else:
