        ...
        if not tf_compatible:
            ...
            def continue_optimization(t, mean, var, best_val, best_sol):
                return tf.logical_and(
                    tf.less(t, self.max_iters), tf.reduce_max(var) > self.epsilon
                )
            ...
            def iteration(t, mean, var, best_val, best_sol):
                ...
                constrained_var = tf.math.minimum(
                    tf.math.minimum(
                        tf.math.square(lb_dist / 2), tf.math.square(ub_dist / 2)
                    ),
                    var,
                )
                ...
                values, indices = tf.math.top_k(-costs, k=self.num_elites, sorted=True)

                best_val, best_sol = tf.cond(
                    tf.math.less(-values[0], best_val),
