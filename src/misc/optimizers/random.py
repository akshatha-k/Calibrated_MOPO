...
class RandomOptimizer(Optimizer):
    ...
    def setup(self, cost_function, tf_compatible):
        ...
        if not tf_compatible:
            ...
            solutions = tf.random_uniform(
                [self.popsize, self.sol_dim], self.ub, self.lb
            )
            costs = cost_function(solutions)
            self.solution = solutions[tf.cast(tf.math.argmin(costs), tf.int32)]
    ...
    def obtain_solution(self, *args, **kwargs):
        ...
        if self.tf_compatible:
            ...
            solutions = np.random.uniform(
                self.lb, self.ub, [self.popsize, self.sol_dim]
            )
