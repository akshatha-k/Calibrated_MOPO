...
from src.modeling.layers.FC import FC
...
class BNN:
    ...
    def __init__(self, params):
        ...
        # Prediction objects
        self.sy_pred_in2d, self.sy_pred_mean2d_fac, self.sy_pred_var2d_fac = (
            None,
            None,
            None,
        )
        self.sy_pred_mean2d, self.sy_pred_var2d = None, None
        self.sy_pred_in3d, self.sy_pred_mean3d_fac, self.sy_pred_var3d_fac = (
            None,
            None,
            None,
        )
    ...
    def train_op(self, sy_train_in, sy_train_targ):
        with tf.name_scope(self.name):
            ...
            train_loss = tf.reduce_sum(
                self._compile_losses(
                    self.sy_train_in, self.sy_train_targ, inc_var_loss=True
                )
            )
            ...
            self.mse_loss = self._compile_losses(
                self.sy_train_in, self.sy_train_targ, inc_var_loss=False
            )

            self.train_op = self.optimizer.minimize(train_loss, var_list=self.optvars)

        with tf.name_scope("calibration"):
            ...
            self.cal_optimizer = tf.keras.optimizers.Adam(learning_rate=5e-2)

            cdf_pred = self.recalibrator(self.sy_cdf_in, activation=False)
            cross_entropy = tf.nn.sigmoid_cross_entropy_with_logits(
                labels=self.sy_cdf_true, logits=cdf_pred
            )
            self.cal_loss = tf.reduce_mean(
                tf.reduce_mean(cross_entropy, axis=-1), axis=-1
            )
    ...
    def save_calibration_info(self, inputs, targets, save_dir, calibrate=True):
        ...
        all_cdfs_pred_cal = (
            self.sess.run(self.recalibrator(all_cdfs_pred)) if calibrate else []
        )
    ...
    def plot_calibration(self, inputs, targets, save_dir):
        ...
        for d in range(all_mus.shape[1]):
            ...
            plt.savefig(
                os.path.join(save_dir, "cal_{}_dim_{}.png".format(timestamp, d))
            )
    ...
    def train(
        ...
        for _ in epoch_range:
            for batch_num in range(int(np.ceil(idxs.shape[-1] / batch_size))):
                ...
                self.sy_train_in = inputs[batch_idxs]
                self.sy_train_targ = targets[batch_idxs]
                train_loss = self.train_op(sy_train_in, sy_train_targ)
                self.optimizer.minimize(train_loss, var_list=self.optvars)
            idxs = shuffle_rows(idxs)
            if not hide_progress:
                if holdout_ratio < 1e-12:
                    epoch_range.set_postfix(
                        {
                            "Training loss(es)": self._compile_losses(
    ...
    def save(self, savedir=None):
        ...
        # Save network parameters (including scalers) in a .mat file
        var_vals = {}
        for i, var_val in enumerate(self.nonoptvars + self.optvars + self.cal_vars):
    ...
    #             kwargs["weight_decay"] = (
    #                 None
    #                 if kwargs["weight_decay"] == "None"
    #                 else float(kwargs["weight_decay"])
    #             )
    #             kwargs["activation"] = (
    #                 None
    #                 if kwargs["activation"] == "None"
    #                 else kwargs["activation"][1:-1]
    #             )
    ...
    def _compile_losses(self, inputs, targets, inc_var_loss=True):
        ...
        if inc_var_loss:
            ...
            total_losses = tf.math.reduce_mean(
                tf.math.reduce_mean(tf.math.square(mean - targets), axis=-1), axis=-1
            )
