...
class MPC(Controller):
    ...
    def __init__(self, params, calibrate=False):
        ...
        self.dO, self.dU = (
            ...
            params.env.action_space.shape[0],
        )
        self.ac_ub, self.ac_lb = (
            params.env.action_space.high,
            params.env.action_space.low,
        )
    ...
    def train(self, obs_trajs, acs_trajs, rews_trajs, logdir=None):
        ...
        if logdir:
            self.model.save_calibration_info(
                new_train_in[0],
                new_train_targs[0],
                logdir,
                calibrate=self.should_calibrate,
            )
        ...
        if self.log_particles:
            if iter_logdir:
                savemat(
                    ...
                    {"predictions": self.pred_particles, "costs": self.pred_costs},
                )
            self.pred_particles = []
            self.pred_costs = []
        elif self.log_traj_preds:
    ...
    def _compile_cost(self, ac_seqs, get_pred_trajs=False):
        ...
        if get_pred_trajs:
            ...
            costs = tf.math.reduce_mean(
                tf.where(tf.is_nan(costs), 1e6 * tf.ones_like(costs), costs), axis=1
            )
    ...
    def _predict_next_obs(self, obs, acs):
        ...
        if self.model.is_tf_model:
            ...
            if self.prop_mode == "TS1":
                proc_obs = tf.reshape(
                    proc_obs, [-1, self.npart, proc_obs.get_shape()[-1]]
                )
                sort_idxs = tf.math.top_k(
                    tf.random_uniform([tf.shape(proc_obs)[0], self.npart]), k=self.npart
                ).indices
                tmp = tf.tile(
                    tf.range(tf.shape(proc_obs)[0])[:, None], [1, self.npart]
                )[:, :, None]
            ...
            if self.model.is_probabilistic and not self.ign_var:
                ...
                if self.prop_mode == "MM":
                    ...
                    predictions = tf.reshape(
                        predictions, [-1, self.npart, model_out_dim]
                    )
            ...
            if self.prop_mode == "TS1":
                ...
                sort_idxs = tf.math.top_k(-sort_idxs, k=self.npart).indices
                idxs = tf.concat([tmp, sort_idxs[:, :, None]], axis=-1)
    ...
    def _expand_to_ts_format(self, mat):
        ...
        return tf.reshape(
            tf.transpose(
                tf.reshape(
                    mat,
                    [-1, self.model.num_nets, self.npart // self.model.num_nets, dim],
                ),
                [1, 0, 2, 3],
            ),
